...
// node_modules/@tonaljs/pitch-distance/node_modules/@tonaljs/pitch-note/dist/index.mjs
...
function transpose(noteName, intervalName) {
  const note5 = note(noteName);
  const intervalCoord = Array.isArray(intervalName) ? intervalName : interval(intervalName).coord;
  if (note5.empty || !intervalCoord || intervalCoord.length < 2) {
    return "";
  }
  const noteCoord = note5.coord;
...
function note2(src) {
  const stringSrc = JSON.stringify(src);
  const cached = cache3.get(stringSrc);
  if (cached) {
    return cached;
  }
  const value2 = typeof src === "string" ? parse3(src) : isPitch(src) ? note2(pitchName3(src)) : isNamedPitch(src) ? note2(src.name) : NoNote2;
  cache3.set(stringSrc, value2);
  return value2;
}
...
var fillStr4 = (character, times) => Array(times + 1).join(character);
var REGEX4 = /^(_{1,}|=|\^{1,}|)([abcdefgABCDEFG])([,']*)$/;
...
function scientificToAbcNotation(str) {
  ...
  const o = oct === 5 ? "" : oct > 4 ? fillStr4("'", oct - 5) : fillStr4(",", 4 - oct);
  return a + l + o;
}
function transpose2(note22, interval2) {
  return scientificToAbcNotation(transpose(abcToScientificNotation(note22), interval2));
}
...
function sortedNoteNames(notes3) {
  const valid = notes3.map((n) => note2(n)).filter((n) => !n.empty);
...
__export(dist_exports2, {
  ...
  note: () => note3,
  pitch: () => pitch,
  stepToLetter: () => stepToLetter3,
  tokenizeInterval: () => tokenizeInterval,
  tokenizeNote: () => tokenizeNote3,
...
var NoNote3 = { empty: true, name: "", pc: "", acc: "" };
var cache4 = /* @__PURE__ */ new Map();
var stepToLetter3 = (step) => "CDEFGAB".charAt(step);
var altToAcc3 = (alt) => alt < 0 ? fillStr5("b", -alt) : fillStr5("#", alt);
var accToAlt3 = (acc) => acc[0] === "b" ? -acc.length : acc.length;
function note3(src) {
  const stringSrc = JSON.stringify(src);
  const cached = cache4.get(stringSrc);
  if (cached) {
    return cached;
  }
  const value2 = typeof src === "string" ? parse4(src) : isPitch(src) ? note3(pitchName4(src)) : isNamedPitch(src) ? note3(src.name) : NoNote3;
  cache4.set(stringSrc, value2);
  return value2;
}
var REGEX5 = /^([a-gA-G]?)(#{1,}|b{1,}|x{1,}|)(-?\d*)\s*(.*)$/;
function tokenizeNote3(str) {
  const m = REGEX5.exec(str);
  return m ? [m[1].toUpperCase(), m[2].replace(/x/g, "##"), m[3], m[4]] : ["", "", "", ""];
}
function coordToNote2(noteCoord) {
  return note3(pitch(noteCoord));
}
var mod3 = (n, m) => (n % m + m) % m;
var SEMI3 = [0, 2, 4, 5, 7, 9, 11];
function parse4(noteName) {
  const tokens = tokenizeNote3(noteName);
  if (tokens[0] === "" || tokens[3] !== "") {
    return NoNote3;
  }
  const letter = tokens[0];
  ...
  const octStr = tokens[2];
  const step = (letter.charCodeAt(0) + 3) % 7;
  const alt = accToAlt3(acc);
  const oct = octStr.length ? +octStr : void 0;
  const coord = coordinates({ step, alt, oct });
  const name3 = letter + acc + octStr;
  const pc = letter + acc;
  const chroma5 = (SEMI3[step] + alt + 120) % 12;
  const height2 = oct === void 0 ? mod3(SEMI3[step] + alt, 12) - 12 * 99 : SEMI3[step] + alt + 12 * (oct + 1);
  const midi3 = height2 >= 0 && height2 <= 127 ? height2 : null;
  const freq2 = oct === void 0 ? null : Math.pow(2, (height2 - 69) / 12) * 440;
  return {
    empty: false,
...
function pitchName4(props) {
  const { step, alt, oct } = props;
  const letter = stepToLetter3(step);
  if (!letter) {
    return "";
  }
  const pc = letter + altToAcc3(alt);
  return oct || oct === 0 ? pc + oct : pc;
}
...
var cache5 = { [EmptyPcset.chroma]: EmptyPcset };
function get(src) {
  ...
  return cache5[chroma22] = cache5[chroma22] || chromaToPcset(chroma22);
}
...
var namedSet = (notes3) => {
  const pcToName = notes3.reduce((record, n) => {
    const chroma5 = note3(n).chroma;
    if (chroma5 !== void 0) {
      record[chroma5] = record[chroma5] || note3(n).name;
    }
    return record;
  }, {});
...
function detect(source, options = {}) {
  const notes3 = source.map((n) => note3(n).pc).filter((x) => x);
  if (note3.length === 0) {
    return [];
  }
...
function findMatches(notes3, weight, options) {
  ...
  const tonicChroma = note3(tonic).chroma;
...
// node_modules/@tonaljs/chord/node_modules/@tonaljs/pitch-note/dist/index.mjs
var fillStr7 = (s, n) => Array(Math.abs(n) + 1).join(s);
var NoNote4 = { empty: true, name: "", pc: "", acc: "" };
var cache6 = /* @__PURE__ */ new Map();
var stepToLetter4 = (step) => "CDEFGAB".charAt(step);
var altToAcc4 = (alt) => alt < 0 ? fillStr7("b", -alt) : fillStr7("#", alt);
var accToAlt4 = (acc) => acc[0] === "b" ? -acc.length : acc.length;
function note4(src) {
  const stringSrc = JSON.stringify(src);
  ...
  const value2 = typeof src === "string" ? parse5(src) : isPitch(src) ? note4(pitchName5(src)) : isNamedPitch(src) ? note4(src.name) : NoNote4;
  cache6.set(stringSrc, value2);
  return value2;
}
var REGEX7 = /^([a-gA-G]?)(#{1,}|b{1,}|x{1,}|)(-?\d*)\s*(.*)$/;
function tokenizeNote4(str) {
  const m = REGEX7.exec(str);
  return m ? [m[1].toUpperCase(), m[2].replace(/x/g, "##"), m[3], m[4]] : ["", "", "", ""];
}
var mod4 = (n, m) => (n % m + m) % m;
var SEMI4 = [0, 2, 4, 5, 7, 9, 11];
function parse5(noteName) {
  const tokens = tokenizeNote4(noteName);
  if (tokens[0] === "" || tokens[3] !== "") {
    return NoNote4;
  }
  const letter = tokens[0];
  ...
  const octStr = tokens[2];
  const step = (letter.charCodeAt(0) + 3) % 7;
  const alt = accToAlt4(acc);
  const oct = octStr.length ? +octStr : void 0;
  const coord = coordinates({ step, alt, oct });
  const name3 = letter + acc + octStr;
  const pc = letter + acc;
  const chroma5 = (SEMI4[step] + alt + 120) % 12;
  const height2 = oct === void 0 ? mod4(SEMI4[step] + alt, 12) - 12 * 99 : SEMI4[step] + alt + 12 * (oct + 1);
  const midi3 = height2 >= 0 && height2 <= 127 ? height2 : null;
  const freq2 = oct === void 0 ? null : Math.pow(2, (height2 - 69) / 12) * 440;
  return {
    empty: false,
...
function pitchName5(props) {
  const { step, alt, oct } = props;
  const letter = stepToLetter4(step);
  if (!letter) {
    return "";
  }
  const pc = letter + altToAcc4(alt);
  return oct || oct === 0 ? pc + oct : pc;
}
...
function add2(intervals2, name3, aliases = []) {
...
data_default2.forEach(
  ([ivls, name3, ...aliases]) => add2(ivls.split(" "), name3, aliases)
);
...
function tokenize2(name3) {
  const [letter, acc, oct, type] = tokenizeNote4(name3);
  if (letter === "") {
    return ["", name3];
  }
  if (letter === "A" && type === "ug") {
...
function getChord(typeName, optionalTonic, optionalRoot) {
  ...
  const tonic = note4(optionalTonic || "");
  const root = note4(optionalRoot || "");
  ...
  const notes3 = tonic.empty ? [] : intervals2.map((i) => transpose(tonic, i));
...
var chord = deprecate2("Chord.chord", "Chord.get", get4);
function transpose3(chordName, interval2) {
  const [tonic, type] = tokenize2(chordName);
  if (!tonic) {
    return chordName;
  }
  return transpose(tonic, interval2) + type;
}
...
var toName = (n) => n.name;
var onlyNotes = (array) => array.map(note3).filter((n) => !n.empty);
...
var get7 = note3;
var name2 = (note5) => get7(note5).name;
var pitchClass = (note5) => get7(note5).pc;
var accidentals = (note5) => get7(note5).acc;
var octave = (note5) => get7(note5).oct;
var midi2 = (note5) => get7(note5).midi;
var freq = (note5) => get7(note5).freq;
var chroma4 = (note5) => get7(note5).chroma;
...
var tr = transpose;
var transposeBy = (interval2) => (note5) => transpose4(note5, interval2);
var trBy = transposeBy;
var transposeFrom = (note5) => (interval2) => transpose4(note5, interval2);
...
var simplify2 = (noteName) => {
  const note5 = get7(noteName);
  if (note5.empty) {
    return "";
  }
  return midiToNoteName(note5.midi || note5.chroma, {
    sharps: note5.alt > 0,
    pitchClass: note5.midi === null
  });
};
...
var cache7 = {};
function get8(src) {
  return typeof src === "string" ? cache7[src] || (cache7[src] = parse6(src)) : typeof src === "number" ? get8(NAMES2[src] || "") : isPitch(src) ? fromPitch(src) : isNamed(src) ? get8(src.name) : NoRomanNumeral;
}
...
function fromPitch(pitch2) {
  return get8(altToAcc3(pitch2.alt) + NAMES2[pitch2.step]);
}
var REGEX9 = /^(#{1,}|b{1,}|x{1,}|)(IV|I{1,3}|VI{0,2}|iv|i{1,3}|vi{0,2})([^IViv]*)$/;
function tokenize3(str) {
  return REGEX9.exec(str) || ["", "", "", ""];
}
...
function majorKey(tonic) {
  ...
  return {
    ...
    alteration,
    keySignature: altToAcc3(alteration),
...
function majorTonicFromKeySignature(sig) {
  if (typeof sig === "number") {
    ...
    return transposeFifths2("C", accToAlt3(sig));
  }
  return null;
}
...
function toRomanNumerals(tonic, chords2) {
  return chords2.map((chord2) => {
    const [note5, chordType2] = tokenize2(chord2);
    const intervalName = distance(tonic, note5);
...
function numeric(notes3) {
  const midi3 = compact2(
    notes3.map((note5) => typeof note5 === "number" ? note5 : toMidi(note5))
  );
  ...
  return midi3.reduce(
    (result, note5) => {
      ...
      return result.concat(range2(last, note5).slice(1));
    },
    [midi3[0]]
  );
}
...
function tokenize4(name3) {
  ...
  const tonic = note3(name3.substring(0, i));
  if (tonic.empty) {
    const n = note3(name3);
...
function get10(src) {
  ...
  const tonic = note3(tokens[0]).name;
...
function detect2(notes3, options = {}) {
  const notesChroma = chroma2(notes3);
  const tonic = note3(options.tonic ?? notes3[0] ?? "");
...
function scaleNotes(notes3) {
  const pcset3 = notes3.map((n) => note3(n).pc).filter((x) => x);
...
function getNoteNameOf(scale2) {
  ...
  const chromas2 = names22.map((name3) => note3(name3).chroma);
  return (noteOrMidi) => {
    const currNote = typeof noteOrMidi === "number" ? note3(fromMidi(noteOrMidi)) : note3(noteOrMidi);
...
function rangeOf(scale2) {
  ...
  return (fromNote, toNote) => {
    const from = note3(fromNote).height;
    const to = note3(toNote).height;
...
function get11(literal) {
  ...
  const ts = build(parse7(literal));
  CACHE.set(stringifiedLiteral, ts);
  return ts;
}
function parse7(literal) {
  if (typeof literal === "string") {
    const [_, up2, low] = REGEX10.exec(literal) || [];
    return parse7([up2, low]);
  }
...
var time_signature_default = { names: names9, parse: parse7, get: get11 };
...
function search(chord2, range3 = defaultRange, dictionary3 = voicing_dictionary_default.triads) {
  ...
  return voicings.reduce((voiced, voicing) => {
    const relativeIntervals = voicing.map(
      (interval2) => interval_default.substract(interval2, voicing[0]) || ""
    );
    const bottomPitchClass = note_default.transpose(tonic, voicing[0]);
    const starts = notesInRange.filter((note5) => note_default.chroma(note5) === note_default.chroma(bottomPitchClass)).filter(
      (note5) => (note_default.midi(
        note_default.transpose(
          note5,
          relativeIntervals[relativeIntervals.length - 1]
        )
      ) || 0) <= (note_default.midi(range3[1]) || 0)
    ).map((note5) => note_default.enharmonic(note5, bottomPitchClass));
    const notes3 = starts.map(
...
export {
  ...
  accToAlt3 as accToAlt,
  altToAcc3 as altToAcc,
  chroma,
  coordToInterval,
  ...
  note3 as note,
  pitch,
  stepToLetter3 as stepToLetter,
  tokenizeInterval,
  tokenizeNote3 as tokenizeNote,
